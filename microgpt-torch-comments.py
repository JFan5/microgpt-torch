...
def gpt(token_ids):
    ...
    x = F.rms_norm(tok_emb + pos_emb, (n_embd,), eps=1e-5)  # joint embedding + normalize | 联合嵌入 + 归一化

    for li in range(n_layer):
        # 1) Multi-head attention block | 多头注意力块
        x_residual = x
        x_norm = F.rms_norm(x, (n_embd,), eps=1e-5)
        # Linear projections for query, key, value | 线性变换得到query、key、value
        q = F.linear(x_norm, state_dict[f'layer{li}.attn_wq'])  # [seq_len, n_embd]
        k = F.linear(x_norm, state_dict[f'layer{li}.attn_wk'])  # [seq_len, n_embd]
        v = F.linear(x_norm, state_dict[f'layer{li}.attn_wv'])  # [seq_len, n_embd]
        ...
        # Scaled dot-product attention with causal mask | 带causal mask的缩放点积注意力
        attn_out = F.scaled_dot_product_attention(q, k, v, is_causal=True)  # [n_head, seq_len, head_dim]
        ...
        x = F.linear(attn_out, state_dict[f'layer{li}.attn_wo']) + x_residual

        # 2) MLP block | MLP块
        x_residual = x
        x = F.rms_norm(x, (n_embd,), eps=1e-5)
        x = F.linear(x, state_dict[f'layer{li}.mlp_fc1'])  # project to 4*n_embd | 映射到4*n_embd维
        x = F.relu(x)                                       # ReLU activation | ReLU激活
        x = F.linear(x, state_dict[f'layer{li}.mlp_fc2'])   # project back to n_embd | 映射回n_embd维
        x = x + x_residual                                  # residual connection | 残差连接

    logits = F.linear(x, state_dict['lm_head'])  # project to vocab_size for next-token prediction | 映射到vocab_size维，用于下一个token的预测
    return logits
